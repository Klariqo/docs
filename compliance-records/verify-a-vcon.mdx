...
Klariqo publishes its signing key, and the verifier confirms a record was signed by that key. This is self-asserted: we vouch for our own key, we are not a certificate authority. The strong, independent guarantee is the tamper check. A valid signature proves the record's contents are exactly what was signed, byte for byte, including the SHA-512 fingerprint of the recording. For an independent attestation that the record existed at a point in time, on a public ledger Klariqo does not control, see [verify a record on the public ledger](/compliance-records/verify-on-the-public-ledger).
