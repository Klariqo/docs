---
title: "Verify a vCon"
description: "Verify any Klariqo compliance record yourself in seconds: check the JWS signature, content hash, and third-party witness entry, right in your browser."
---
...
Klariqo publishes its signing key, and the verifier confirms a record was signed by that key. This is self-asserted: we vouch for our own key, we are not a certificate authority. The strong, independent guarantee is the tamper check. A valid signature proves the record's contents are exactly what was signed, byte for byte, including the SHA-512 fingerprint of the recording. For an independent attestation that the record existed at a point in time, see [verify a record on the public ledger](/compliance-records/verify-on-the-public-ledger). That check runs against a public ledger Klariqo does not control.
